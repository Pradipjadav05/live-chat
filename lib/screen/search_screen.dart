        ...
        child: Column(
          children: [
            CustomTextFormField(
              ...
              hintText: 'Search',
              // call the filtered user list methods
              onChanged: (val) => Provider.of<FirebaseProvider>(context, listen: false).searchUser(val),
            ),
            Consumer<FirebaseProvider>(
              builder: (context, value, child) => Expanded(
                ...
                // build ui of filtered user list
                    : ListView.builder(
                        padding: const EdgeInsets.all(16),
                        itemCount: value.search.length,
                        itemBuilder: (context, index) =>
                        //build filtered user list but not include signIn user
                            value.search[index].uid != FirebaseAuth.instance.currentUser?.uid
